...
contract DepositPaymaster is BasePaymaster {
    ...
    address[] public whiteList = new address[](6);
    mapping(address => bool) public isWhiteList;

    constructor(IEntryPoint _entryPoint,address[] memory _whiteList) BasePaymaster(_entryPoint) {
    ...
    function addToken(IERC20 token, IOracle tokenPriceOracle) external onlyOwner {
        ...
        require(isWhiteList[address(token)]);
    ...
    internal view override returns (bytes memory context, uint256 validationData) {
        ...
        IERC20 token = IERC20(address(bytes20(paymasterAndData[20:])));//@note 顺序调换
